...
class UpsertInvestmentAssetInteractorTest : DescribeSpec({
    ...
    describe("happy path") {
        ...
        beforeEach {
            every { repository.upsertAsset(any(), any(), any(), any(), any(), any(), any()) } returns Unit.right()
        }

        it("returns Right(Unit) when all inputs are valid") {
            interactor.execute("AAPL", "Apple Inc.", "Technology", "NASDAQ", 1000.0, 1200.0, null)
                .shouldBeRight()
        }

        it("delegates to the repository with the exact values provided") {
            interactor.execute("MSFT", "Microsoft Corp.", "Technology", "NASDAQ", 500.0, 600.0, null)

            verify(exactly = 1) {
                repository.upsertAsset("MSFT", "Microsoft Corp.", "Technology", "NASDAQ", 500.0, 600.0, null)
            }
        }

        it("accepts zero as a valid investedAmount") {
            interactor.execute("AAPL", "Apple Inc.", "Technology", "NASDAQ", 0.0, 0.0, null)
                .shouldBeRight()
        }

        it("accepts zero as a valid currentValue") {
            interactor.execute("AAPL", "Apple Inc.", "Technology", "NASDAQ", 100.0, 0.0, null)
                .shouldBeRight()
        }
        ...
        it("propagates a DatabaseError returned by the repository") {
            ...
            every { repository.upsertAsset(any(), any(), any(), any(), any(), any(), any()) } returns error.left()

            interactor.execute("AAPL", "Apple Inc.", "Technology", "NASDAQ", 100.0, 120.0, null)
    ...
    describe("validation — ticker") {
        ...
        it("returns InvalidParameter when ticker is blank") {
            interactor.execute("   ", "Apple Inc.", "Technology", "NASDAQ", 100.0, 120.0, null)
                .shouldBeLeft()
        ...
        it("does not call the repository when ticker is blank") {
            ...
            verify(exactly = 0) { repository.upsertAsset(any(), any(), any(), any(), any(), any(), any()) }
        }
    }
    ...
    describe("validation — name") {
        ...
        it("returns InvalidParameter when name is blank") {
            interactor.execute("AAPL", "   ", "Technology", "NASDAQ", 100.0, 120.0, null)
                .shouldBeLeft()
                .shouldBeInstanceOf<InvestmentError.InvalidParameter>()
    ...
    describe("validation — sector") {
        ...
        it("returns InvalidParameter when sector is blank") {
            interactor.execute("AAPL", "Apple Inc.", "   ", "NASDAQ", 100.0, 120.0, null)
                .shouldBeLeft()
                .shouldBeInstanceOf<InvestmentError.InvalidParameter>()
    ...
    describe("validation — investedAmount") {
        ...
        it("returns InvalidParameter when investedAmount is negative") {
            interactor.execute("AAPL", "Apple Inc.", "Technology", "NASDAQ", -0.01, 100.0, null)
                .shouldBeLeft()
                .shouldBeInstanceOf<InvestmentError.InvalidParameter>()
        ...
        it("returns InvalidParameter when investedAmount is a large negative value") {
            interactor.execute("AAPL", "Apple Inc.", "Technology", "NASDAQ", -9999.0, 100.0, null)
                .shouldBeLeft()
                .shouldBeInstanceOf<InvestmentError.InvalidParameter>()
        }

        it("does not call the repository when investedAmount is negative") {
            interactor.execute("AAPL", "Apple Inc.", "Technology", "NASDAQ", -1.0, 100.0, null)
    ...
    describe("validation — currentValue") {
        ...
        it("returns InvalidParameter when currentValue is negative") {
            ...
            interactor.execute("AAPL", "Apple Inc.", "Technology", "NASDAQ", 100.0, -0.01, null)
                .shouldBeLeft()
                .shouldBeInstanceOf<InvestmentError.InvalidParameter>()
        ...
        it("does not call the repository when currentValue is negative") {
            interactor.execute("AAPL", "Apple Inc.", "Technology", "NASDAQ", 100.0, -50.0, null)
